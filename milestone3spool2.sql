SQL> select xmlroot(xmlelement("studentlist",xmlagg(xmlelement("Faculty",xmlattributes(s.foid.faculty_id as "code"),xmlelement("name",s.foid.faculty_name),xmlagg(xmlelement("student",xmlattributes(s.soid.sid as "sn"),xmlelement("Sname",s.soid.sname),xmlelement("Surname",s.soid.slname)))))), version '1.0') as doc from section_table s where s.coid.cid = 'ITEC1001' group by s.foid.faculty_id, s.foid.faculty_name;
...
SQL>
SQL> select xmlelement("Transcript",xmlattributes(t.transcript_id as "TID"),xmlforest(t.csid.coid.cname as "program", t.csid.foid.faculty_name as "faculty",t.soid.sid as "sn")) from transcript_table t;
...
SQL>
SQL>
SQL> select xmlroot(xmlelement("StudentList",xmlagg(xmlelement("Department",xmlattributes(g.doid.dep_id as "ID"),xmlagg(xmlelement("Student",xmlattributes(g.sid as "sn"),xmlelement("Name",g.fullname()),xmlelement("Degree",g.degree)))))),version '1.0') as doc from graduate_table g where g.gpa > 2.5 group by g.doid.dep_id;
...
SQL> select xmlelement("Student",xmlelement("sn",s.soid.sid),xmlelement("Fname",s.soid.sname),xmlelement("Surname",s.soid.slname),xmlelement("GPA",s.soid.gpa),xmlelement("Department",s.foid.doid.dep_name)) as doc from section_table s where s.location = 'DB001';
...
SQL> select xmlelement("Section",xmlattributes(s.section_id as "SCID"),xmlelement("name",s.foid.doid.dep_name),xmlelement("Faculty",s.foid.faculty_name),xmlelement("Day",s.day)) as doc from csection s where s.soid.sname = 'Brad' and s.soid.slname = 'Pitt';
...
SQL>
SQL>
SQL> select xmlelement("Student",xmlattributes(s.soid.sid as "ID"),xmlelement("Sname",s.soid.sname),xmlelement("Surname",s.soid.slname),xmlelement("Section",s.section_id)) as doc from csection s where s.day = 'Wednesday';
...
SQL>
SQL>
SQL> select xmlroot(xmlelement("Courselist",xmlagg(xmlelement("DepartmentID",xmlattributes(c.doid.dep_id as "ID"),xmlagg(xmlelement("CourseID",xmlattributes(c.cid as "ID"),xmlelement("Name",c.cname),xmlelement("AdminID",c.doid.adid.admin_id)))))),version '1.0') as doc from course_table c where c.credit = 3 group by c.doid.dep_id;
...
SQL>
SQL>
SQL> select xmlroot(xmlelement("Receipt",xmlagg(xmlelement("Payment",xmlattributes(p.pay_id AS "ID"),(select xmlagg(xmlelement("Transcript",xmlattributes(p2.ts.transcript_id AS "ID"),xmlagg(xmlelement("Student",xmlattributes(p2.ts.soid.sid AS "ID"),xmlelement("FirstName", p2.ts.soid.sname),xmlelement("LastName", p2.ts.soid.slname)))))from payment_table p2 where p2.pay_id = p.pay_id group by p2.ts.transcript_id)))), version '1.0') as doc from payment_table p where p.amount = 500 group by p.pay_id;
...
SQL> xquery
  ...
  3  where $student/@ID = 'S005' or $student/@ID = 'S006' or $student/@ID = 'S007'
  4  return
  5   <Student>
...
SQL> xquery
  ...
  3    let $faculty := doc('/public/mil3/faculty.xml')//FacultyID[FNAME = 'Engineering']
  4    return
...
SQL> xquery
  ...
  3    let $faculty := doc('/public/mil3/faculty.xml')//FacultyID[FNAME = 'Health']
  4    return
  5    2    3    4    5      ($students/Sname/text(), $students/Email/text(), $faculty/FNAME/text())
  6  /
ERROR:
ORA-19114: XPST0003 - error during parsing the XQuery expression:
